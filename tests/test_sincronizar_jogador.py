...
def test_sincronizar_jogador_avulso_com_usuario():
    ...
    nome_avulso_test = f"Luan Avulso {uniq}"
    nome_user_test = f"Luan Silva {uniq}"
    username_test = f"luanteste_{uniq}"
    email_test = f"luan.{uniq}@exemplo.com"
    ...
    try:
        ...
        assert resultado['nome_avulso'] == nome_avulso_test
        ...
        assert j_sorteio['nome'] == nome_user_test
        ...
        assert part_user['jogador_nome'] == nome_user_test
    finally:
        # Cleanup: remover dados de teste
        if avulso:
            try:
                jog_svc.deletar(avulso.id)
            except Exception:
                pass
        if usuario:
            try:
