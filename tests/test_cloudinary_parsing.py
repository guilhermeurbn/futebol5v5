...
def test_cloudinary_three_vars_parsing():
    dummy_img = Image.new("RGB", (100, 100), color="blue")
    img_byte_arr = io.BytesIO()
    dummy_img.save(img_byte_arr, format="PNG")
    img_byte_arr.seek(0)
    ...
    env = {
        "CLOUDINARY_CLOUD_NAME": "nt547vdb",
        "CLOUDINARY_API_KEY": "834268265253525",
        "CLOUDINARY_API_SECRET": "GaVdaTINIw0dXb7FIkRkZ0Bfgm4"
    }
    ...
    with patch.dict(os.environ, env, clear=True):
        with patch("cloudinary.uploader.upload") as mock_upload:
            ...
            us = UploadService()
            url_resultado = us.processar_foto_perfil(file_storage, user_id="user_test_456")
